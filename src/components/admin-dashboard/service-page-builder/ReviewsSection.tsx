...
import { useSaveServicePageReviews } from '@/lib/hooks/admin/service-page-reviews';
import { ServicePageReviewsDTO } from '@/lib/api/admin/service-page-reviews.api';

interface ReviewsSectionProps {
  servicePageId: string;
  initialData?: ServicePageReviewsDTO | null;
}
...
export default function ReviewsSection({
  ...
  const [eyeTextNl, setEyeTextNl] = useState(initialData?.eye_text_nl || '');
  const [eyeTextEn, setEyeTextEn] = useState(initialData?.eye_text_en || '');

  const saveMutation = useSaveServicePageReviews();
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setEyeTextNl(initialData.eye_text_nl || '');
      setEyeTextEn(initialData.eye_text_en || '');
    }
  }, [initialData]);
      ...
      >
        <div className='flex items-center gap-3'>
          ...
          <h3 className='text-lg font-semibold text-slate-900'>
            {locale === 'nl' ? 'Reviews Sectie' : 'Reviews Section'}
          </h3>
          {(headingNl || headingEn || eyeTextNl || eyeTextEn) && (
          ...
          <div className='grid grid-cols-2 gap-6'>
            ...
            <div className='space-y-4 p-4 bg-white rounded-lg border border-slate-200'>
              ...
              <div className='space-y-2'>
                ...
                <Input
                  placeholder='bijv. Klantverhalen'
                  value={headingNl}
              ...
              <div className='space-y-2'>
                <label className='block text-sm font-medium text-slate-900'>
                  {locale === 'nl' ? 'Oogtekst (max 15 tekens)' : 'Eye Text (max 15 chars)'}
                </label>
                <Input
                  placeholder='bijv. Klantverhalen'
                  value={eyeTextNl}
                  onChange={(e) => setEyeTextNl(e.target.value.slice(0, 15))}
                  className='bg-slate-50 border-slate-300'
                  maxLength={15}
                />
                <p className='text-xs text-slate-500'>{eyeTextNl.length}/15</p>
              </div>
            </div>
            ...
            <div className='space-y-4 p-4 bg-white rounded-lg border border-slate-200'>
              ...
              <div className='space-y-2'>
                ...
                <Input
                  placeholder='e.g. Customer Stories'
                  value={eyeTextEn}
                  onChange={(e) => setEyeTextEn(e.target.value.slice(0, 15))}
                  className='bg-slate-50 border-slate-300'
                  maxLength={15}
                />
                <p className='text-xs text-slate-500'>{eyeTextEn.length}/15</p>
              </div>
            </div>
          </div>
